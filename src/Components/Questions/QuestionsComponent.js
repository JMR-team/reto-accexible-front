...
function QuestionsComponent(props) {
  ...
  let [optionIsSelectedArray, setOptionIsSelectedArray] = useState([
    false,
    false,
    false,
    false,
  ...
  return (
    <>
      <div className="quiz-body">
        {questions.length > 0 &&
          questionIndex < questions.length && ( // comprobamos si questions tiene elementos dentro y si la variable questionIndex es menor que la longitud del array de questions
            <div className="quiz-modal">
              <div className="quiz-modal-header">
                <label className="quiz-modal-title">
                  {questions[questionIndex].question}{" "}
                </label>
              </div>
              <br />
              {questions[questionIndex].answers.map(function (
                answerIndividual,
                index
              ) {
                return (
                  <>
                    <button
                      className={
                        optionIsSelectedArray[index]
                          ? "selected"
                          : "quiz-modal-option"
                      }
                      onClick={() =>
                        selectAnswer(
                          answerIndividual.text,
                          answerIndividual.score,
                          index
                        )
                      }
                    >
              ...
              <div className="quiz-modal-footer">
                <div className="quiz-modal-progress-controls">
                  <p className="quiz-modal-progress-current">{`${
                  ...
                  <button
                    className={
                      "quiz-modal-btn" +
                      (okButtonIsActive ? "" : " quiz-modal-btn-disabled")
                    }
                    onClick={okButtonIsActive ? () => nextQuestion() : null}
                  >
                    {" "}
                    Síguiente{" "}
                  </button>
  ...
  function selectAnswer(answer, score, index) {
    let previouslySelectedIndex = optionIsSelectedArray.findIndex(
      (item) => item
    );
    if (previouslySelectedIndex >= 0)
      optionIsSelectedArray[previouslySelectedIndex] = false;
